 ...
 * The versions folder must be in same folder... Still working on a workaround -_-'
 */
...
public class start{
	...
	static MouseListener ml = new MouseListener(){
		...
		public void mousePressed(MouseEvent e) {
			if (e.isShiftDown()&&(e.getButton()==1)){ busy=false;
				//Debug enabled
				if (debug) {debug=false;log("Debug off...",false);}
	...
	static void write(String name){
		...
		update = null;
			...
			int ext = extraction.extractTo(name.substring(x,name.length())+"/"+"minecraft.jar", loc.getParentFile().getPath()+"/");
			if (ext==-1){//Failed...
